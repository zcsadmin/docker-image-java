...
: "${DOCKER_USER:?DOCKER_USER env var must be set}"
: "${DOCKER_GROUP:?DOCKER_GROUP env var must be set}"

DOCKER_USER_UID=$(id -u "${DOCKER_USER}")
DOCKER_GROUP_GID=$(id -g "${DOCKER_GROUP}")

FIX_PATH=(/app "/home/${DOCKER_USER}")
...
if [[ ! -z "$FIX_UID" ]] && [[ "$FIX_UID" != "$DOCKER_USER_UID" ]]; then
    ...
    if [ ! -z "$PREV_USER" ]; then
        ...
        find "${FIX_PATH[@]}" -user "${FIX_UID}" -exec chown -h 888 {} \; 2>/dev/null || true
        usermod -u 888 "$PREV_USER"
    fi

    find "${FIX_PATH[@]}" -user "${DOCKER_USER_UID}" -exec chown -h "${FIX_UID}" {} \; 2>/dev/null || true
...
if [[ ! -z "$FIX_GID" ]] && [[ "$FIX_GID" != "$DOCKER_GROUP_GID" ]]; then
    ...
    if [ ! -z "$PREV_GROUP" ]; then
        ...
        find "${FIX_PATH[@]}" -group "${FIX_GID}" -exec chgrp -h 888 {} \; 2>/dev/null || true
        groupmod -g 888 "$PREV_GROUP"
    fi

    find "${FIX_PATH[@]}" -group "${DOCKER_GROUP_GID}" -exec chgrp -h "${FIX_GID}" {} \; 2>/dev/null || true
